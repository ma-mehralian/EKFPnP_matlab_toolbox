...
name = {'EKFPnP', 'PPnP','MLPnP','EPnP', 'RPnP', 'ASPnP', 'OPnP', 'DLS', 'CEPPnP', 'SRPnP'};
f = {[], @PPnP, @MLPNP_with_COV, @EPnP_planar,  @RPnP, @ASPnP, @OPnP, @robust_dls_pnp, @CEPPnP_planar, @SRPnP};
marker = { 'x', '<', 'd', '*', 's', 'v', 'o', '+', '>', '^'};
...
for i= 1:length(nls)
    ...
    for j= 1:num
        ...
        for h=1:cam_num
            ...
            for id = 1:npt
                Cu(:,:,id) = diag([(nl_e*nls(i))^2 (nl_e*nls(i))^2]);
                cov_proj = K\diag([(nl_e*nls(i))^2 (nl_e*nls(i))^2 0])/K';
                J = (eye(3)-(v(:,id)*v(:,id)')/(v(:,id)'*v(:,id)))/norm(homx(:,id));
                Evv(:,:,id) = J*cov_proj*J';
                cov(:,id) = reshape(Evv(:,:,id),9,1);
            end
            RC = (nl_e*nls(i))*eye(2*npt);
